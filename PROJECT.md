...
- [x] **生成图片直接下载**：已实现
- [x] **删除旧 bundle**：已完成
  - [x] **批量导入独立 Tab**：从"对话设置"中分离为独立的第三个 Tab，提升长表单场景的可用性
  - [x] **批量导入 emoji 选择器**：预设 emoji 直接插入文本，自定义表情包插入 [表情:N] 占位符
  - [x] **批量导入 [表情:N] 命令**：batchImport 解析 [表情:N] 映射到 customEmojis[N-1]
  - [x] **预设自定义表情包**：public/static/emojis/ 目录下的图片自动加载为自定义表情
- [x] **组件拆分**：已完成
